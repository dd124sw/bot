...
client.on("guildMemberAdd", (member) => {
  ...
  member.addRole(guild.roles.find(role => role.name == "크루원"));
});
...
client.on('message', (message) => {
  ...
  if(message.content == 'embed') {
    ...
    let embed = new Discord.RichEmbed()
      ...
      .setAuthor('나긋해', img, 'http://www.naver')
      .setThumbnail(img)
      .addBlankField()
      .addField('Inline field t', 'Some value here')
  ...
  if(message.content.startsWith('어몽봇 전체공지')) {
    if(checkPermission(message)) return
    if(message.member != null) { // 채널에서 공지 쓸 때
      let contents = message.content.slice('어몽봇 전체공지'.length);
  ...
  if(message.content.startsWith('어몽봇 청소')) {
    ...
    var clearLine = message.content.slice('어몽봇 청소 '.length);
